...
// 🟡 SELECCIÓN DE CLIENTE POR ARGUMENTO
const cliente = process.argv[2];
if (!cliente || !['nuevaeps', 'compensar', 'mutualser','saludmia'].includes(cliente)) {
  console.error('❌ Debes especificar el cliente como argumento: nuevaeps o compensar');
  console.error('Ejemplo: node script.js nuevaeps');
  process.exit(1);
}
...
for (let i = START; i <= END; i++) {
  ...
  const EXCEL_FILE = `./${cliente}/${NUM_FACTURA}.xlsx`;
  const OUTPUT_JSON = `./${cliente}/${NUM_FACTURA}_final.json`;
...
// 🟡 LECTURA DE USUARIOS DESDE EXCEL
function leerUsuariosDesdeExcel(ruta) {
  ...
  datos.forEach(fila => {
    ...
    const consulta = {
      codPrestador: String(fila.codPrestador ?? '').trim(),
      fechaInicioAtencion: formatearFecha(fila.fechaInicioAtencion, 'YYYY-MM-DD HH:mm'),
      numAutorizacion: String(fila.numAutorizacion ?? '').trim(),
      codConsulta: String(fila.codConsulta ?? '').trim(),
      ...
      finalidadTecnologiaSalud: String(fila.finalidadTecnologiaSalud ?? '').trim(),
      causaMotivoAtencion: String(fila.causaMotivoAtencion ?? '').trim(),
